# 网易企业邮箱邮件线程分析 MCP Server

这是一个本地运行的只读 MCP Server，用于让 Codex 读取网易企业邮箱中的邮件线程，并辅助分析一封邮件或完整邮件线程里不同人员提出的需求、问题、待确认事项，最后整理“应该回复谁、回复哪些点”。

## 项目目标

第一版 MVP 只做一条链路：

网易企业邮箱 IMAP 只读读取 -> 搜索邮件 -> 读取单封邮件 -> 尽量重建完整邮件线程 -> 输出稳定 thread JSON -> 交给 Codex 根据固定 prompt 做邮件需求拆解。
...
```bash
python -m netease_mail_mcp.cli test-connection
```

列出邮箱目录：

```bash
python -m netease_mail_mcp.cli list-mailboxes
```
...
```bash
python -m netease_mail_mcp.cli search --from customer@example.com --subject "CES" --limit 10
```

读取单封邮件：

```bash
python -m netease_mail_mcp.cli read --email-id "12345"
```

拉取完整线程：

```bash
python -m netease_mail_mcp.cli thread --email-id "12345" --output thread.json
```

判断收件箱邮件是否已经回复：

```bash
python -m netease_mail_mcp.cli reply-status --since 2026-06-01 --limit 50 --output reply-status.json
```

如果已发送目录不是 `Sent`，先用 `list-mailboxes` 找到已发送目录的 `select_name`，然后：

```bash
python -m netease_mail_mcp.cli reply-status --sent-mailbox "&XfJT0ZAB-" --since 2026-06-01
```

输出固定分析 prompt：

```bash
python -m netease_mail_mcp.cli print-prompt
```
...
```toml
command = "/path/to/netease-mail-mcp/.venv/bin/python"
```
